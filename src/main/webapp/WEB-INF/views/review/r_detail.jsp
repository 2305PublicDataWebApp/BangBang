		...
		<div class="center">
			...
			<div class="button-1">
				<c:if test="${review.rUserId eq userId }">
					...
					<button class="modibu" type="button"
						onclick="showModifyPage('${modifyUrl }');">수정하기</button>
					<button class="delbu" type="button"
						onclick="deleteReview('${deleteUrl}');">삭제하기</button>
				</c:if>
			...
			<hr>
			<div>
				<img class="reimg" alt="첨부파일"
					src="/resources/ruploadFiles/${review.reviewFileRename }"
			...
			<form action="/rreply/rr_insert.do" method="post">
				...
				<table>
					<tr>
						<td><textarea rows="4" cols="100" name="rReplyContent"
								class="textre"></textarea></td>
						<td><input class="replymodi" type="submit" value="등록">
						</td>
					</tr>
				</table>
			</form>
				...
				<table class="replyta">
					<c:forEach var="rreply" items="${rrList }">
						<tr>
							...
							<input type="hidden" name="rrAdminId"
								value="${rreply.rrAdminId }">
							<td><c:if test="${rreply.rrUserId eq userId}">
									<a class="mo" href="javascript:void(0);"
										onclick="showModifyForm(this, '${rreply.rReplyContent }');">수정하기</a>
									...
									<a href="javascript:void(0);"
										onclick="deleteReply('${delUrl}');">삭제하기</a></td>
						</tr>
						</c:if>
						<c:if test="${adminId ne null }">
							<c:url var="delUrl" value="/rreply/rr_delete.do">
						...
						<tr id="rReplyModifyForm" style="display: none;">
							<td><input class="modite" id="rReplyContent" type="text"
								size="50" name="rReplyContent" value="${rreply.rReplyContent }"></td>
							<td><input type="button"
								onclick="rReplyModify(this,'${rreply.rReplyNo}', '${rreply.rReviewNo }');"
								value="완료"></td>
						</tr>
						<br />
					</c:forEach>
